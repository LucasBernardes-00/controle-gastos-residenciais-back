...
namespace WebApplication1.UseCases.Transactions
{
    public class SaveTransactionUseCase
    {
        ...
        public async Task<TransactionResponse> Execute(TransactionSaveRequest req)
        {
            ...
            Transaction transaction = new Transaction { Id = Guid.NewGuid(), Description = req.Description, Amount = req.Amount, Type = req.Type, CategoryId = req.CategoryId, PersonId = req.PersonId };

            await _repository.Save(transaction);
            return new TransactionResponse { Id = transaction.Id, Description = transaction.Description, Amount = transaction.Amount, Type = transaction.Type, Category = category, Person = person };
        }

        private void Validate(TransactionSaveRequest req, ValidationResult validation, Category category, Person person)
        {
